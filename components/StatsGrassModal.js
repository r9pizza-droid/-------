                                                ...
                                                <div className="space-y-3 max-h-60 overflow-y-auto custom-scroll">
                                                    {taskComments.length > 0 ? (
                                                        ...
                                                        <div className="flex flex-col items-center justify-center py-10 px-4 text-center bg-slate-50/50 rounded-2xl border border-dashed border-slate-200">
                                                            ...
                                                            <h4 className="text-xs font-bold text-slate-700 mb-1">작성된 과제 메모가 없습니다</h4>
                                                            <p className="text-[10px] text-slate-500 mt-1">체크리스트의 연필 아이콘을 눌러<br/>학생을 위한 짧은 메모를 남겨보세요.</p>
                                                        </div>
                                                    )}
                                                </div>
                                            </div>
                        ...
                        {isInsightLoading ? (
                            ...
                            {missedTags.length > 0 ? (
                                ...
                                <div className="flex flex-col items-center justify-center py-10 px-4 text-center bg-slate-50/50 rounded-2xl border border-dashed border-slate-200 mt-2">
                                    <div className="mb-4"><span className="text-5xl inline-block animate-bounce drop-shadow-md">🎉</span></div>
                                    <h4 className="text-sm font-bold text-slate-700 mb-1">미흡한 과목이 없습니다</h4>
                                    <p className="text-xs text-slate-500 mt-1">모든 과목의 과제를 훌륭하게 수행하고 있어요!</p>
                                </div>
                            )}
                        </div>
                        )}
                                                ...
                                                <div className="flex items-center gap-2">
                                                    ...
                                                    <div className="relative">
                                                        <button
                                                            ...
                                                            className="text-[10px] font-bold text-slate-600 bg-slate-50 hover:bg-slate-100 border border-slate-200 rounded-md px-2 py-1 outline-none cursor-pointer transition-colors flex items-center gap-1"
                                                        >
                                                            <Icon d={PATHS.calendar} size={10} />
                                                            {selectedScoreDate ? dayjs(selectedScoreDate).format('MM.DD (ddd)') : '날짜 선택'}
                                                        </button>
                                                        {showScoreCal && (
                                                            <div className="absolute right-0 top-full mt-1 bg-white border border-slate-100 shadow-2xl shadow-indigo-100/50 rounded-2xl p-3 z-[100] w-60 animate-fade-in cursor-default" onClick={e => e.stopPropagation()}>
                                                                <div className="flex justify-between items-center mb-3 px-1">
                                                                    <button onClick={() => setScoreCalMonth(scoreCalMonth.subtract(1, 'month'))} className="p-1 rounded-full hover:bg-slate-100 text-slate-400 hover:text-slate-700 transition-colors"><Icon d={PATHS.left} size={14} /></button>
                                                                    <span className="font-extrabold text-xs text-slate-800">{scoreCalMonth.format('YYYY년 M월')}</span>
                                                                    <button onClick={() => setScoreCalMonth(scoreCalMonth.add(1, 'month'))} className="p-1 rounded-full hover:bg-slate-100 text-slate-400 hover:text-slate-700 transition-colors"><Icon d={PATHS.right} size={14} /></button>
                                                                </div>
                                                                ...
                                                                <div className="grid grid-cols-7 gap-1 text-center text-xs">
                                                                    ...
                                                                    {Array.from({ length: scoreCalMonth.daysInMonth() }).map((_, i) => {
                                                                        ...
                                                                        let cellClass = "aspect-square flex flex-col items-center justify-center rounded-full text-[10px] font-medium transition-all duration-200 relative group cursor-pointer ";
                                                                        if (isSelected) cellClass += " bg-indigo-600 text-white shadow-sm font-bold scale-105 z-10";
                                                                        ...
                                                                        return (
                                                                            <button key={i} onClick={() => { setSelectedScoreDate(dateStr); setShowScoreCal(false); }} className={cellClass}>
                                                                                <span>{i + 1}</span>
                                                                                {hasTask && !isSelected && <div className={`absolute bottom-0.5 left-1/2 -translate-x-1/2 w-1 h-1 rounded-full ${isToday ? 'bg-indigo-400' : 'bg-blue-400'}`}></div>}
                                                                            </button>
                                                                        );
                                                                    })}
                                                                </div>
                                        ...
                                        <div className="absolute top-full left-1/2 -translate-x-1/2 mt-3 w-64 bg-white rounded-2xl shadow-xl border border-slate-100 p-4 z-50 animate-fade-in" onClick={e => e.stopPropagation()}>
                                            ...
                                            <div className="space-y-3 max-h-48 overflow-y-auto custom-scroll">
                                                ...
                                                {combinedNotes.filter(n => n.content && (n.content.includes('칭찬') || n.content.includes('스티커') || n.content.includes('상점'))).length === 0 && (
                                                    <div className="flex flex-col items-center justify-center py-10 px-2 text-center bg-slate-50/50 rounded-xl border border-dashed border-slate-200">
                                                        <div className="mb-3"><span className="text-4xl inline-block animate-bounce drop-shadow-md">🌟</span></div>
                                                        <h4 className="text-xs font-bold text-slate-700 mb-1">칭찬 기록이 아직 없습니다</h4>
                                                        <p className="text-[10px] text-slate-500 mt-1">학생에게 따뜻한 칭찬 한마디를 남겨주세요.</p>
                                                    </div>
                                                )}
                                            </div>
                        ...
                        {isInsightLoading ? (
                            ...
                            <div className="flex-1 overflow-y-auto custom-scroll pr-1 max-h-[400px]">
                                {displayedNotes.length > 0 ? (
                                    ...
                                    <div className="h-56 flex flex-col items-center justify-center py-8 px-4 text-center bg-slate-50/50 rounded-2xl border-2 border-dashed border-slate-200 mt-2">
                                        <div className="mb-5"><span className="text-5xl inline-block animate-bounce drop-shadow-md">📂</span></div>
                                        <h4 className="text-sm font-bold text-slate-700 mb-1">아직 활동 기록이 없습니다</h4>
                                        <p className="text-xs text-slate-500 mt-1">하단의 입력창을 이용해<br/>학생의 첫 번째 기록을 남겨보세요.</p>
                                    </div>
                                )}
                                {hasMore && (
                            ...
                            <div className="grid grid-cols-1 grid-rows-1">
                                ...
                                <div className={`col-start-1 row-start-1 w-full transition-all duration-300 ease-in-out transform ${isDetailViewOpen ? 'translate-x-0 opacity-100 visible' : 'translate-x-full opacity-0 invisible pointer-events-none'}`}>
                                    ...
                                    className="w-full p-2 border border-slate-200 rounded-lg text-xs outline-none focus:border-indigo-500 focus:ring-4 focus:ring-indigo-500/20 transition-all duration-300 resize-none"
                                                            ...
                                                            <div className="flex flex-col items-center justify-center py-14 bg-slate-50/50 rounded-2xl border-2 border-dashed border-slate-200 text-center gap-2">
                                                                <div className="mb-3"><span className="text-5xl inline-block animate-bounce drop-shadow-md">📭</span></div>
                                                                <h4 className="text-sm font-bold text-slate-700 mb-1">등록된 과제가 없습니다</h4>
                                                                <p className="text-xs text-slate-500">선택하신 날짜에는 부여된 과제가 없어요.</p>
                                                            </div>
                                                        )}
                                                    </section>
